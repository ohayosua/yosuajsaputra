...
- Data on customer actions. Examples include clicks, visits, etc.
	- These were my initial thoughts as to calculating the engagement rate...


Ways to create engagement rate (Aiming for 10% engagement rate):
- Look into boosting content/marketing ads for lower engaged groups of customers.
- Segment the customers for insights that could lead to good business decisions: Targeting specific email campaigns for the specifically divided groups.
...
- Divide customer list into smaller targeted groups to create more effective email marketing campaigns. 
	e.g (engagement level: low, avg, high), (types of spenders: low, average, heavy), etc..
...
deduped_sales as (
	select distinct * 
	from sales),
-- Removing duplicate customer ID records (Customers with two addresses logged) in the customers table. 
-- For simplicty in this project, we will include only the first city that occurs in the dupicate records as I will assume it is a typo until further context given.
